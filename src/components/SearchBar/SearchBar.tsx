  ...
  async function searchCity(cityName: string, country: string[]) {
    ...
    try {
      if (country[1] !== 'default' && country[0] === 'user') {
        cities = await axios.get(`${process.env.REACT_APP_BACKEND}/cities?name=${cityName}&country=${country[1]}`, { cancelToken: newSource.token })
      } else {
        cities = await axios.get(`${process.env.REACT_APP_BACKEND}/cities?name=${cityName}`, { cancelToken: newSource.token })
      }
      setLoading(false);
